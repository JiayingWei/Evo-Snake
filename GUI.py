...
class Boxy:
	...
	def randomMove(self, screenWidth, screenHeight):
		"""moves a box to a random location
		"""
...
class Snake:
	...
	def move(self,direction):
		""" Moves the snake by removing last element in snake.boxlist and adding a new box in the direction of motion
		"""
		increment = 20
		if direction == 'up':
	...
	def collisions(self,boxlist1,boxlist2,screenwidth,screenheight):		#where boxlist1 is the snake and boxlist2 is whatever box we're checking against
		...
		"""
		if boxlist1[0].x < 0 or boxlist1[0].x + 20 > screenwidth or boxlist1[0].y < 0 or boxlist1[0].y > screenheight: #detects collision with wall
			return 'ouch'
			...
			for boxy in boxlist2:
				for box in boxlist1:
					if box != boxy and box.x == boxy.x and box.y == boxy.y:
						return 'ouch'
		elif len(boxlist2) == 1 and boxlist1[0].x == boxlist2[0].x and boxlist1[0].y == boxlist2[0].y: #detects contact with food
...
class EvoSnakeView:
	...
	def drawLoading(self):
		...
		for i in range(0,10): #fills the loading bar in 1 second
	...
	def drawGame(self):
		...
		#resets to menu states if there is a collision
		...
		#checks for food collisions
			...
			for i in range (len(self.model.menu.snake.boxlist)):
				coordlist.append((self.model.menu.snake.boxlist[i].x,self.model.menu.snake.boxlist[i].y)) #makes a list containing all coordinates of snake boxes
			if (self.model.menu.surpriseBoxy.x,self.model.menu.surpriseBoxy.y) in coordlist: #checks if food was spawned inside the snake

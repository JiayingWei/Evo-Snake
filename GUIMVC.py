...
class Menu:
	...
	def Minimized(self, width = 600, height = 600):
	...
	def Loading(self):
		...
		"""
		self.boxy = Boxy()  #creates a Loading boxy
...
class EvoSnakeView:
	...
	def drawMenu(self):
		...
		for i in range(0,10):
			loading_boxy_start = centerObject(self.model.menu.width, self.model.menu.height, self.model.menu.boxy.width * 10, self.model.menu.boxy.height)
			loading_boxy_start = [loading_boxy_start[0] + self.model.menu.boxy.width * i, loading_boxy_start[1]]
			pygame.draw.rect(self.screen, self.model.menu.boxy.color, (loading_boxy_start,self.model.menu.boxy.size),0)
			pygame.display.update()
